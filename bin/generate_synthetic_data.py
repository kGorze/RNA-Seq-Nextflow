...
def parse_args():
    ...
    parser.add_argument(
        "--num_reads", type=int, default=10000, help="Number of reads per sample"
    )
    parser.add_argument("--read_length", type=int, default=100, help="Length of reads")
    parser.add_argument(
        "--genome_size", type=int, default=50000, help="Size of synthetic genome"
    )
    parser.add_argument(
        "--num_genes", type=int, default=200, help="Number of genes in synthetic genome"
    )
    ...
    with open(gtf_file, "w") as f:
        ...
        for gene_id in range(1, num_genes + 1):
            ...
            for tx_id in range(1, num_transcripts + 1):
                ...
                # Transcript entry
                f.write(
                    f"chr1\tsynthetic\ttranscript\t{tx_start}\t{tx_end}\t.\t{strand}\t.\t"
                )
                f.write(
                    f'gene_id "gene{gene_id}"; transcript_id "{transcript_id}"; gene_name "Gene{gene_id}";\n'
                )
                ...
                for exon_id in range(1, num_exons + 1):
                    ...
                    # Exon entry
                    f.write(
                        f"chr1\tsynthetic\texon\t{exon_start}\t{exon_end}\t.\t{strand}\t.\t"
                    )
                    f.write(
                        f'gene_id "gene{gene_id}"; transcript_id "{transcript_id}"; exon_number "{exon_id}"; gene_name "Gene{gene_id}";\n'
                    )
...
def create_readme(output_dir):
    ...
    with open(readme_file, "w") as f:
        f.write(
            """# Synthetic RNA-Seq Pipeline Test Dataset
...
def main():
    ...
    sample_ids = generate_fastq_files(
        args.output_dir, args.num_samples, args.num_reads, args.read_length
    )
