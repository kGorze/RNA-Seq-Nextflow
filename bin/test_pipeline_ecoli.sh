...
# Script to test the RNA-Seq Nextflow pipeline with the E. coli test dataset
# This script runs the pipeline with the E. coli test dataset for validation

# Set variables
PIPELINE_DIR=$(pwd)
TEST_DATA_DIR="${PIPELINE_DIR}/test_data"
RESULTS_DIR="${PIPELINE_DIR}/results_ecoli_test"

# Check if test data exists, if not prepare it
if [ ! -d "${TEST_DATA_DIR}" ]; then
    echo "Test data directory not found. Preparing E. coli test dataset..."
    bash bin/prepare_test_data.sh
fi

# Run the pipeline with the E. coli test dataset
echo "Running RNA-Seq pipeline with E. coli test dataset..."
...
if [ $? -eq 0 ]; then
    echo "Pipeline test with E. coli dataset completed successfully!"
    ...
    echo "Pipeline test with E. coli dataset failed!"
    exit 1
fi
